...
describe("Photo", () => {
    ...
    describe("POST api/v1/photo", () => {
        ...
        describe("EMPTY BODY", () => {
            it("PROVIDE TOKEN CASE WITH (all) EMPTY BODY", async () => {
                ...
                expect(body).toHaveProperty("msg", "Photo must be tied by User, however User your provided does not exist");
            });
    ...
    describe("GET api/v1/photo", () => {
        it("NOT PROVIDE TOKEN CASE - should return 401 unauthorized", async () => {
            const { body, statusCode } = await supertest(app).get("/api/v1/photo");
            expect(statusCode).toBe(401);
            expect(body).toHaveProperty("msg");
        });
        it("PROVIDE INCORRECT TOKEN CASE - should return 500 and got jsonwebtokenerror message", async () => {
        ...
        it("PROVIDE CORRECT TOKEN CASE AND INCLUDE USER - should return 200 and get all photos", async () => {
            const { body, statusCode } = await supertest(app)
                .get("/api/v1/photo?include=User")
                .set("Authorization", `Bearer ${user.token}`);
            expect(statusCode).toBe(200);
            expect(body).toHaveProperty("photos[0].User");
        });
    });
    describe("GET api/v1/photo/:id", () => {
        it("NOT PROVIDE TOKEN CASE - should return 401 unauthorized", async () => {
            const { body, statusCode } = await supertest(app).get(`/api/v1/photo/${123}`);
            expect(statusCode).toBe(401);
            expect(body).toHaveProperty("msg");
        });
        ...
        it("PROVIDE CORRECT TOKEN CASE, FOUND ID AND INCLUDE USER - should return 200 and get photo", async () => {
            const { body, statusCode } = await supertest(app)
                .get(`/api/v1/photo/${photoId}?include=User`)
                .set("Authorization", `Bearer ${user.token}`);
            expect(statusCode).toBe(200);
            expect(body).toHaveProperty("photo");
            expect(body).toHaveProperty("photo.User");
        });
    });
});
